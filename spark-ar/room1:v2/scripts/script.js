...
Diagnostics.log( Scene.root.find('tvdis').text.pinLastValue());
...
if((result.status >=200) && (result.status <300)){
    ...
    throw new Error('HTTP Status Code :'+ result.status);
      ...
      if(json["status"]!="success") {
        ...
        Scene.root.find('tvprice').text = "Rs."+productsjson[1]["price"].toString();
        Scene.root.find('tvname').text = productsjson[1]["product.name"];
        Scene.root.find('camprice').text = "Rs."+productsjson[2]["price"].toString();
        Scene.root.find('camname').text = productsjson[2]["product.name"];
        Scene.root.find('lapprice').text = "Rs."+productsjson[0]["price"].toString();
        Scene.root.find('lapname').text = productsjson[0]["product.name"];
        Scene.root.find('tvdis').text = productsjson[1]["description"];
        Scene.root.find('camdis').text = productsjson[2]["description"];
...
Time.setTimeout(() => {
  Scene.root.find('canvas0').hidden = true;
  getData();
}, 5000);